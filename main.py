...
# Update ChessEngine initialization
c_engine = ChessEngine(resource_path('chess.exe'))
...
piece_movement_sound_path = resource_path("images/pieces/piece_movement_sound.wav")

piece_movement_sound = pygame.mixer.Sound(piece_movement_sound_path)
piece_movement_sound.set_volume(0.3)

game_win_sound = pygame.mixer.Sound(piece_movement_sound_path)
...
for piece in PIECE_NAMES:
    ...
    try:
        PIECE_IMAGES[ piece ] = pygame.transform.scale(
            pygame.image.load(resource_path(f'images/pieces/{piece}.png')),
...
def draw_chessboard(win):
    ...
    colors = [constants.COLORS["CREAM"], constants.COLORS["BROWN"]]
    ...
    for row in range( ROWS ):
        ...
        for col in range( COLS ):
            ...
            if highlights[row][col] == '1': 
                ...
                highlight_surface.fill( constants.COLORS["HIGHLIGHT"] )
...
def draw_turn_indicator( win ):
    ...
    if game_state.current_player == 'P':
        ...
        text = font.render( "White's Turn", True, constants.COLORS["WHITE"] )

    # otherwise do the same for black
    else:
        text = font.render( "Black's Turn", True, constants.COLORS["BLACK"] )
...
async def main( c_engine ):
    ...
    while True:
        ...
        if game_state.state == LOGIN_MENU:
            ...
            logged_in = menu.login_menu(
                WIN, WIDTH, HEIGHT, constants.COLORS["BROWN"], constants.COLORS["CREAM"], constants.COLORS["BLACK"], constants.COLORS["WHITE"], game_state, logged_in
            )
        ...
        if game_state.state == MENU:
            ...
            menu.main_menu(WIN, WIDTH, HEIGHT, game_state, c_engine)
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if game_state.game_type == 1 and game_state.current_player == 'O':
            ...
            # check for mouseclick
                ...
                # otherwise, its in the moving state (post selecting)
                    ...
                    if game_state.new_board and not boards_are_equal(game_state.new_board, game_state.board):
                        ...
                        # Play piece movement sound here
